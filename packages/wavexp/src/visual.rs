...
impl SoundVisualiser {
    ...
    pub const fn canvas(&self) -> &NodeRef {
        &self.canvas
    }
    ...
    pub fn handle_event(&mut self, event: &AppEvent, sequencer: &Sequencer) {
        match event {
            AppEvent::Resize => {
                let canvas: HtmlCanvasElement = self.canvas.cast()?;
            ...
            AppEvent::Frame(..) => {
                if sequencer.playback_ctx().playing() {
                    ...
                    let out = unsafe {
                        from_raw_parts(
                            self.out_data.as_ptr().cast(),
                            self.out_data.len().checked_mul(4)?,
                        )
                    };
...
impl HintHandler {
    ...
    pub fn handle_event(&mut self, event: &AppEvent) {
        match event {
            ...
            AppEvent::FetchHint(e) => {
                ...
                let mut src: Element = e.target_dyn_into()?;
    ...
    pub const fn main_bar(&self) -> &NodeRef {
        &self.main_bar
    }
...
pub trait GraphPoint: Sized + Clone + Ord + 'static {
    ...
    const OFFSET_Y_BOUND: RangeV2<R64>;
...
impl<T: GraphPoint> GraphEditor<T> {
    pub fn new(data: Vec<T>) -> Self {
        let res = Self {
            ...
            inner: AnyGraphEditor {
                scale: const {
                    [
                        R64::new_or(
                            T::SCALE_X_BOUND.start,
                            T::SCALE_X_BOUND.start.get() * 0.75 + T::SCALE_X_BOUND.end.get() * 0.25,
                        ),
                        R64::new_or(
                            T::SCALE_Y_BOUND.start,
                            T::SCALE_Y_BOUND.start.get() * 0.75 + T::SCALE_Y_BOUND.end.get() * 0.25,
                        ),
                    ]
                },
        ...
        GRAPH_EDITOR_COUNT.set(res.id.wrapping_add(1));
        res
    }
    ...
    pub fn remove_points(
        ...
        for index in to_remove.rev() {
            ...
            let (rem, was_selected) = loop {
                ...
                match index.cmp(x) {
                    Ordering::Less => *x = x.wrapping_sub(1), // will never underflow
                    Ordering::Equal => {
                        break unsafe {
                            let x = ids_iter.len();
    ...
    pub fn init(&mut self) {
        ...
        canvas.set_width(w);
        canvas.set_height(h);
        self.scale = self.scale.ceil_to([r64!(2); 2]);
        self.grid = None;
        if self.offset.x <= 0 {
            self.offset.x = (T::OFFSET_X_BOUND.start * (w as f32) / self.scale[0]).into()
        }
        if self.offset.y <= 0 {
        ...
        match ctx.special_action() {
            SpecialAction::Select => {
                ...
                self.inner.selection = self
                    ...
                    .enumerate()
                    .filter_map(|(id, x)| {
                        match x.in_hitbox(&area, ctx.as_ref(), sequencer, *visual_ctx) {
                            Ok(true) => Some(Ok(id)),
                            Ok(false) => None,
                            Err(e) => Some(Err(e)),
                        }
                ...
                let prev_size = replace(
                    ...
                    area.map(|RangeInclusiveV2 { start, end }| end - start),
                );
        ...
        match &mut self.inner.focus {
            ...
            Focus::Zoom {
                ...
                if cursor.left {
                    ...
                    let point = cursor.point.add(*init_offset)?.sub(*pivot)?;
            ...
            Focus::Point {
                ...
                if cursor.left {
                    ...
                    self.set_zoom_focus(cursor)?
                } else if !self.data.get(*id)?.in_hitbox(
                    &cursor_point_user.map(|x| x.incl_range_to(x)),
                    ctx.as_ref(),
                    sequencer,
                    *visual_ctx,
                )? {
        ...
        match event {
            ...
            AppEvent::Frame(_) if self.redraw => {
                ...
                let offset_y = R64::from(-self.offset.y)
                    % if self.offset.y > (T::Y_BOUND.start * step[1]).into() {
                        step[1] * 2
                    } else {
                        R64::INFINITY
                    };
